...
MAX = 5  # maximal interval limit for interpolation
IMODE = 1  # cross-correlation mode: 0, twice (default);  1, interpolate light curve 1; 2, interpolate light curve 2.
...
interp = 0.1 #Interpolation time step (days). Must be less than the average cadence of the observations, but too small will introduce noise.
...
##########################################
tlag_peak, status_peak, tlag_centroid, status_centroid, ccf_pack, max_rval, status_rval, pval = myccf.peakcent(mjd1, flux1, mjd2, flux2, lag_range[0], lag_range[1], interp, MAX, imode=IMODE)
tlags_peak, tlags_centroid, nsuccess_peak, nfail_peak, nsuccess_centroid, nfail_centroid, max_rvals, nfail_rvals, pvals = myccf.xcor_mc(mjd1, flux1, abs(err1), mjd2, flux2, abs(err2), lag_range[0], MAX, lag_range[1], interp, nsim = nsim, mcmode=mcmode, sigmode = 0.2, imode=IMODE)
...
### Calculate the best peak and centroid and their uncertainties using the median of the distributions. 
...
print('Peak, errors: %10.3f  (-%10.3f +%10.3f)'%(peaktau, peaktau_loerr, peaktau_uperr))
...
output_file_folder = './results/{IMODE}/'.format(IMODE=IMODE)
